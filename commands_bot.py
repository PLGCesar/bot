...
from database import DONO_BOT_ID, db_get, db_set, obter_registro, obter_ou_auto_registrar, obter_senha_admin, logger, TEMAS_DISPONIVEIS, obter_tema
...
class PainelAdminView(discord.ui.View):
    ...
    async def status_button(self, inter, button):
        lat = f"{self.bot.latency * 1000:.0f}ms" if self.bot.is_ready() else "N/A"; usr = len(db_get("users", {})); nxt = db_get("global_config/proximo_bot_id", 1)
        embed = discord.Embed(title="🖥️ Status Detalhado", color=discord.Color.blue()).add_field(name="Ping", value=f"`{lat}`").add_field(name="Guildas", value=f"`{len(self.bot.guilds)}`").add_field(name="Registros", value=f"`{usr}`").add_field(name="Próximo ID", value=f"`#{nxt}`")
    ...
    async def ver_registros_button(self, inter, button):
        db = db_get("users", {})
        if not db or not isinstance(db, dict): await inter.response.send_message("🔍 Sem registros.", ephemeral=True); return
        lista = [f"• ID Bot: `#{v.get('bot_id', '?')}` | `{v.get('nome', 'Desconhecido')}` (Discord: <@{d}>)" for d, v in db.items()]
        txt = "\n".join(lista[:15])
        if len(lista) > 15: txt += f"\n*... e outros {len(lista) - 15} registros.*"
        await inter.response.send_message(embed=discord.Embed(title="👥 Usuários Cadastrados", description=txt, color=discord.Color.green()), ephemeral=True)
...
class BotCommands(commands.Cog):
    ...
    async def registrar_config_slash(self, inter, permissao: bool = None, campo_index: int = None, campo_nome: str = None):
        ...
        if campo_index is not None and campo_nome is not None:
            if campo_index < 0 or campo_index > 5: await inter.response.send_message("❌ Índice inválido.", ephemeral=True); return
            lbls = cfg.get("labels", {})
            lbls[str(campo_index)] = campo_nome
            cfg["labels"] = lbls
        db_set(f"server_config/{g_id}", cfg)
        await inter.response.send_message("✅ Configurações atualizadas!", ephemeral=True)
    ...
    async def perfil_config_prefix(self, ctx, opcao: str = None, *, valor: str = None):
        ...
        db_set(f"users/{ctx.author.id}/perfil", p); await ctx.send("✅ Perfil atualizado!")
    ...
    async def perfil_config_slash(self, inter, fundo: str = None, fundo_url: str = None, posicao: str = None, descricao: str = None):
        ...
        db_set(f"users/{inter.user.id}/perfil", p); await inter.response.send_message("✅ Perfil atualizado!", ephemeral=True)
    ...
    async def monitorar_slash(self, inter, user_id: str):
        if not inter.guild: await inter.response.send_message("❌ Apenas em guilda.", ephemeral=True); return
        guild_id = str(inter.guild.id); obter_ou_auto_registrar(inter.user, guild_id)
