...
from database import (
    DONO_BOT_ID, db_get, db_set, obter_registro, obter_ou_auto_registrar,
    obter_senha_admin, logger
)
from utils import (
    obter_tema, gerar_imagem_perfil, TEMAS_DISPONIVEIS, rolar_dado_viciado,
    deserializar_permissoes_canal, extrair_estrutura_completa_servidor
)
...
class ColarConfirmacaoView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.autor_id:
            await interaction.response.send_message("❌ Apenas quem disparou o comando original pode autorizar.", ephemeral=True)
            return False
        return True
...
class PainelAdminView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.autor_id:
            await interaction.response.send_message("❌ **Acesso negado!** Apenas o administrador master pode usar os botões.", ephemeral=True)
            return False
        return True
    ...
    async def sync_gist_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            from database import sincronizar_banco_local
...
# --- CLASSE COG DE COMANDOS (ATIVOS) [3] ---

class BotCommands(commands.Cog):
    def __init__(self, bot_inst: commands.Bot):
        self.bot = bot_inst
    ...
    # --- COMANDOS: DIAGNÓSTICO I/O ---
    @commands.command(name="io-write")
    async def io_write_prefix(self, ctx: commands.Context, *, texto: str = None):
    ...
    async def io_write_slash(self, interaction: discord.Interaction, texto: str):
    ...
    async def io_read_prefix(self, ctx: commands.Context):
    ...
    async def io_read_slash(self, interaction: discord.Interaction):
        t = db_get("teste/mensagem")
    ...
    async def registrar_prefix(self, ctx: commands.Context):
    ...
    async def registrar_slash(self, interaction: discord.Interaction):
    ...
    async def registrar_servidor_prefix(self, ctx: commands.Context, membro: discord.Member = None, *, nomes_raw: str = None):
    ...
    async def registrar_servidor_slash(self, interaction: discord.Interaction, membro: discord.Member, nome: str, nome1: str="Não Definido", nome2: str="Não Definido", nome3: str="Não Definido", nome4: str="Não Definido", nome5: str="Não Definido"):
    ...
    async def registrar_config_prefix(self, ctx: commands.Context, sub_comando: str = None, *args):
    ...
    async def registrar_config_slash(self, interaction: discord.Interaction, permissao: bool = None, campo_index: int = None, campo_nome: str = None):
        if not interaction.guild: await interaction.response.send_message("❌ Apenas em guilda.", ephemeral=True); return
    ...
    async def perfil_prefix(self, ctx: commands.Context):
        u = obter_ou_auto_registrar(ctx.author, str(ctx.guild.id) if ctx.guild else "DM")
        t = obter_tema(u)
    ...
    async def perfil_slash(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True)
    ...
    async def perfil_tema_prefix(self, ctx: commands.Context, tema: str = None):
        if not tema:
    ...
    async def perfil_tema_slash(self, interaction: discord.Interaction, tema: str = None):
        if not tema:
    ...
    async def perfil_config_prefix(self, ctx: commands.Context, opcao: str = None, *, valor: str = None):
    ...
    async def perfil_config_slash(self, interaction: discord.Interaction, fundo: str = None, fundo_url: str = None, posicao: str = None, descricao: str = None):
    ...
    async def senha_adm_prefix(self, ctx: commands.Context):
    ...
    async def senha_adm_slash(self, interaction: discord.Interaction):
    ...
    async def painel_adm_prefix(self, ctx: commands.Context):
    ...
    async def painel_adm_slash(self, interaction: discord.Interaction):
        if interaction.user.id != DONO_BOT_ID: await interaction.response.send_message("❌ Dono apenas.", ephemeral=True); return
        await interaction.response.send_message(embed=discord.Embed(title="⚙️ Painel Master", description="Escolha uma opção:", color=discord.Color.dark_red()), view=PainelAdminView(self.bot, interaction.user.id), ephemeral=True)

    # --- COMANDOS: COPIAR E COLAR ---
    @commands.command(name="copiar")
    async def copiar_prefix(self, ctx: commands.Context):
        if not ctx.guild: await ctx.send("❌ Apenas em guilda."); return
    ...
    async def copiar_slash(self, interaction: discord.Interaction):
        if not interaction.guild: await interaction.response.send_message("❌ Apenas em guilda.", ephemeral=True); return
    ...
    async def colar_prefix(self, ctx: commands.Context):
        if not ctx.guild: await ctx.send("❌ Apenas em guilda."); return
    ...
    async def colar_slash(self, interaction: discord.Interaction):
        if not interaction.guild: await interaction.response.send_message("❌ Apenas em guilda.", ephemeral=True); return
        ...
        await interaction.response.send_message(embed=discord.Embed(title="⚠️ AVISO DE CLONAGEM ⚠️", description="Você criará cargos, canais e categorias salvos.\nDeseja iniciar?", color=discord.Color.orange()), view=ColarConfirmacaoView(self.bot, interaction.user.id, d, interaction.guild), ephemeral=True)

    # --- COMANDOS: ANTIRAID ---
    @commands.command(name="monitorar")
    async def monitorar_prefix(self, ctx: commands.Context, user_id: str = None):
    ...
    async def monitorar_slash(self, interaction: discord.Interaction, user_id: str):
        if not interaction.guild: await interaction.response.send_message("❌ Apenas em guilda.", ephemeral=True); return
    ...
    async def desmonitorar_prefix(self, ctx: commands.Context, user_id: str = None):
        obter_ou_auto_registrar(ctx.author, str(ctx.guild.id) if ctx.guild else "DM")
        if not (ctx.author.guild_permissions.administrator or ctx.author.id == ctx.guild.owner_id or ctx.author.id == DONO_BOT_ID):
            await ctx.send("❌ Apenas administradores."); return
        if not user_id: await ctx.send("❌ Forneça o ID."); return
    ...
    async def desmonitorar_slash(self, interaction: discord.Interaction, user_id: str):
        if not interaction.guild: await interaction.response.send_message("❌ Apenas em guilda.", ephemeral=True); return
        guild_id = str(interaction.guild.id); obter_ou_auto_registrar(interaction.user, guild_id)
        if not (interaction.user.guild_permissions.administrator or interaction.user.id == interaction.guild.owner_id or interaction.user.id == DONO_BOT_ID):
            await interaction.response.send_message("❌ Apenas administradores.", ephemeral=True); return
